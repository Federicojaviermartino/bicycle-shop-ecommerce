...
class MockApiService {
  ...
  private loadCartsFromStorage(): void {
    try {
      ...
      if (stored) {
        this.mockCarts = JSON.parse(stored);
      }
    } catch {
      this.mockCarts = {};
    }
  ...
  private loadConfigurationsFromStorage(): void {
    try {
      const stored = localStorage.getItem('bicycle-shop-configurations');
      if (stored) {
        this.mockConfigurations = JSON.parse(stored);
      }
    } catch {
  ...
  private saveCartsToStorage(): void {
    try {
      localStorage.setItem('bicycle-shop-carts', JSON.stringify(this.mockCarts));
    } catch {
      // Storage quota exceeded or unavailable
    }
  }

  private saveConfigurationsToStorage(): void {
    try {
      localStorage.setItem('bicycle-shop-configurations', JSON.stringify(this.mockConfigurations));
    } catch {
      // Storage quota exceeded or unavailable
    }
  }
