...
export class ProductConfigurationService {
  ...
  public async getAvailableOptions(
    ...
    for (const option of allOptions) {
      ...
      const isAvailable = await this.isOptionAvailable(option, currentSelections, constraints);
      if (isAvailable) {
  ...
  public async createConfiguration(
    ...
    const totalPrice = validation.isValid ? await this.calculatePrice(productId, selections) : 0;
    ...
    for (const constraint of constraints) {
      ...
      for (const rule of rules) {
        if (rule.targetPartOptionId === option.id && rule.ruleType === 'DISABLES') {
          const triggerSelected = currentSelections.some(
            (s) => s.partOptionId === rule.triggerPartOptionId
          );
        ...
        case 'REPLACE':
          if (!effect.targetPartOptionId) {
            ...
            const targetSelection = selections.find(
              (s) => s.partOptionId === effect.targetPartOptionId
            );
            if (targetSelection) {
