...
export class PartRepository {
  ...
  async getPartOption(id: string): Promise<PartOption | null> {
    const result = await this.db.query<PartOption>('SELECT * FROM part_options WHERE id = $1', [
      id,
    ]);
    return result.rows[0] || null;
  }
  ...
  async updatePartOptionStock(
    id: string,
    inStock: boolean,
    stockCount?: number
  ): Promise<PartOption | null> {
    const result = await this.db.query<PartOption>(
...
export class ConstraintRepository {
  ...
  async createConstraint(
    constraint: Omit<ConfigurationConstraint, 'id'>
  ): Promise<ConfigurationConstraint> {
    const result = await this.db.query<ConfigurationConstraint>(
